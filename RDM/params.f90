...
integer :: Ntime                     ! number of timesteps for memory kernal 
real*8  :: TimeStep, ConvTol         ! Timestep for memory kernal, Convergrence tolerance for memory kernel
...
complex*16, allocatable :: Kk(:,:,:)                      !Memory kernel 
...
read(555,'(a75)') infostr
read(555,*) TimeStep, Ntime, ConvTol
...
allocate(KK(Nel**2,Nel**2,0:Ntime))
...
open(1005,file='KReal.out',status='old')
open(2005,file='KImag.out',status='old')
