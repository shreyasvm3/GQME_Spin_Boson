...
allocate(Kold(Nel**2,Nel**2,0:Ntime),intg(0:Ntime))
...
do while (flag.eq.1)
  ...
  do i = 0, Ntime
    ...
    do j = 0, i
       ! Get the integrand
       intg(j) = Fk(1,1,i-j)*Kold(1,1,j)
    end do
    !integrate
    call TrapRule(0,i,intg(0:i),Timestep,Kk(1,1,i)) 
  end do
...
real*8                  :: dx
complex*16,intent(in)   :: fx(xmin:xmax)
integer                 :: i
complex*16,intent(out)  :: intgrl
